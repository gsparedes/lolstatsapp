...
exports.getSummonerChampionData = function(summonerId, callback) {
	var summonerChampions = [];
	async.waterfall([
		function(cb) {
			var path = 'v1.3/stats/by-summoner/' + summonerId + '/ranked?season=SEASON2016&';
			httpClient(path, function(err, data) {
				if (err)
					return cb(err);
		...
		function(champions, cb) {
      		async.sortBy(champions, function(champion, sortCB) {
      			sortCB(null, champion.totalSessionsPlayed);
			}, function(err, results) {
				cb(null, results);
			});
      	},
